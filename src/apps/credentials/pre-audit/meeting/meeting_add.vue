    ...
    .then((result) => {
      ...
      data.map((item) => {
        if (pushedPackId.indexOf(item.fPackage.fId) < 0) {
          ...
          formValue.cities.push(item.fPackage.fId)
          formValue.citiesBak.push(item.fPackage.fId)
          formValue.packageIds.push({
            fId: item.fPackage.fId,
          })
          formValue.packageIndexs.push(item.fPackage.fIndex)
...
const saveData = async () => {
  //校验是否选择包件
  if (formValue.packageIds.length === 0) {
    message.error('请先选择包件！')
    return
  }
  try {
  ...
  if (formValue.fMeetingType.fName.indexOf('资格') >= 0) {
    oaForm.meetingType = '1'
  } else {
...
function init() {
  ...
  //读取一下项目编号
  var projectId = props.data.fId
  try {
    api.projectMainLoad({ fId: projectId }).then(function (res) {
      if (res.status == 200) {
