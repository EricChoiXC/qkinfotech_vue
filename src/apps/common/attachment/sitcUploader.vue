<!--
	...
	入参:
		...
		linkPackage:必填，是否关联包件，默认false
		closeUploadModal：关闭调用的方法
 -->
<template>
  <div class="upload-container">
    <label ref="uploadLabel" class="uploader">
      <input
        style="display: none"
        type="file"
        ref="uploadFile"
    ...
    <table class="file-list">
      <thead>
        <tr class="center-text">
          <th>文件名</th>
          <th v-if="$props.linkPackage">包件</th>
          <th>大小</th>
          <th>状态</th>
          <th>操作</th>
        </tr>
      ...
      <tbody>
        ...
        >
          <td>{{ fileInfo.name }}</td>
          <td v-if="$props.linkPackage">
            <n-grid cols="4">
              <n-gi>
                <n-checkbox-group
                  v-model:value="fileInfo.clickAll"
                  @update:value="clickPackageAll(fileInfo)"
                >
              ...
              <n-gi v-for="item in packages">
                <n-checkbox-group
                  v-model:value="fileInfo.packages"
                  @update:value="clickPackage(fileInfo)"
                >
                  <n-space justify="start">
                    <n-checkbox :value="item.value">
                      包件{{ item.index }}:{{ item.name }}
                    </n-checkbox>
                  </n-space>
                </n-checkbox-group>
              </n-gi>
          ...
          <td>
            <n-space>
              ...
              <n-button
                ...
                >删除</n-button
              >
              <n-button v-else @click="removeFile(fileInfo)" type="error"
                >删除</n-button
              >
            </n-space>
          </td>
...
const props = defineProps(['linkPackage', 'modelId', 'modelName', 'fkey', 'fileType', 'multiple'])
const emit = defineEmits(['saveAttachment', 'closeUploadModal', 'init'])

//包件
const packages = inject('attachmentPackages')
...
async function upload(fileList) {
  ...
  files.forEach((file) => {
    let fileInfo = reactive({
      ...
      progress: 0,
      status: 'waiting',
      packages: packages.value.map((val) => val.value),
      clickAll: ['All'],
    })
    //校验：未允许格式
    if (props.fileType) {
...
function removeFile(fileInfo) {
  ...
  const nameIndex = addFileNames.indexOf(fileInfo.name)
  if (nameIndex > -1) {
    addFileNames.splice(nameIndex, 1)
  }
}
...
async function confirm() {
  try {
    ...
    if (props.linkPackage) {
      if (!checkHasPackage(fileInfoList.value)) {
        message.error('附件必须至少关联一个包件！', {
          duration: 0,
          closable: true,
        })
        return
    ...
    saveAttachment(fileInfoList.value)
  } catch (e) {
    console.log(e)
  }
}
...
//校验是否有勾选包件
function checkHasPackage(list) {
  for (var i = 0; i < list.length; i++) {
    if (list[i].packages.length == 0) {
      return false
    }
  }
  return true
...
async function saveAttachment(fileList) {
  await emit('saveAttachment', fileList)
}

function close() {
  dialog.warning({
    title: '警告',
    content: '关闭页面，附件不会进行保存，是否关闭？',
    positiveText: '关闭',
    negativeText: '取消',
...
//点击包件
function clickPackage(fileInfo) {
  if (fileInfo.packages.length === packages.value.length) {
    fileInfo.clickAll = ['All']
  } else {
    fileInfo.clickAll = []
  }
}
...
//点击全选包件
function clickPackageAll(fileInfo) {
  if (fileInfo.clickAll.includes('All')) {
    fileInfo.packages = packages.value.map((val) => val.value)
  } else {
    fileInfo.packages = []
  }
}
