    ...
    <n-layout>
      <n-layout-content>
        ...
        <n-space justify="end" style="width: 100%">
          ...
          <commonImport
            module="masterdata/scale"
            class="com.qkinfotech.core.tendering.model.masterModels.MasterDataScale"
            @closeImportModalFunc="onCloseImportModal"
          ></commonImport>
          <commonExport module="masterdata/scale"></commonExport>
        </n-space>
    ...
    render(row) {
      return [
        h(
          ...
          {
            onClick: () => {
              // const url = `/opdesk/sys/master/masterDataScaleView?fId=${encodeURIComponent(row.fId)}`;
              // window.open(url);
              layout.open({
                label: '查看规模',
        ...
        h(
          ...
          {
            onClick: () => {
              // const url = `/opdesk/sys/master/masterDataScaleEdit?fId=${encodeURIComponent(row.fId)}`;
              // window.open(url);
              layout.open({
                label: '编辑规模',
...
const add = () => {
  // const url = `/opdesk/sys/master/masterDataScaleAdd`;
  // window.open(url);
...
const handleChildWindowClose = (event) => {
  if (event.data.action === 'scaleWindowClose') {
    actions.list()
  }
}
