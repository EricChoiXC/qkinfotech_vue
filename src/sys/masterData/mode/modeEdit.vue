...
async function handleSubmit() {
  try {
    ...
    if (res.data.list.length > 0 && res.data.list[0].fId != id.value) {
...
const id = masterData / meetingType / meetingTypeAdd.vue // 使用 ref 来创建响应式数据
// 使用 watch 监听路由参数的变化
// watch(
//   () => route.query.fId,
//   (newFId) => {
