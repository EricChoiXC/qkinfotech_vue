    ...
    <n-layout>
      <n-layout-content>
        ...
        <n-space justify="end" style="width: 100%">
          ...
          <commonImport
            module="masterdata/nature"
            class="com.qkinfotech.core.tendering.model.masterModels.MasterDataNature"
            @closeImportModalFunc="onCloseImportModal"
          ></commonImport>
          <commonExport module="masterdata/nature"></commonExport>
        </n-space>
...
const columns = ref([
  ...
  render(row) {
    return [h(
      ...
      onClick: () => {
        // console.log(row.fId);
        // const url = `/opdesk/sys/master/masterDataNatureView?fId=${encodeURIComponent(row.fId)}`;
        // window.open(url);
        layout.open({
          label: "查看性质",
        ...
        h(
          NButton,
          {
            onClick: () => {
              // console.log(row.fId);
              // const url = `/opdesk/sys/master/masterDataNatureEdit?fId=${encodeURIComponent(row.fId)}`;
              // window.open(url);
              layout.open({
                label: '性质编辑',
...
const add = () => {
  // const url = `/opdesk/sys/master/masterDataNatureAdd`;
  // window.open(url);
...
const handleChildWindowClose = (event) => {
  if (event.data.action === 'natureWindowClose') {
    // 这里调用你的刷新列表的方法
    actions.list()
  }
}
