    ...
    <n-layout>
      <n-layout-content>
        ...
        <n-space justify="end" style="width: 100%">
          ...
          <commonImport
            module="masterdata/category"
            class="com.qkinfotech.core.tendering.model.masterModels.MasterDataCategory"
            @closeImportModalFunc="onCloseImportModal"
          ></commonImport>
          <commonExport module="masterdata/category"></commonExport>
        </n-space>
        <div
          style="
            text-align: center;
    ...
    render(row) {
      return [
        h(
          ...
          {
            onClick: () => {
              // const url = `/opdesk/sys/master/masterDataCategoryView?fId=${encodeURIComponent(row.fId)}`;
              // window.open(url);
              layout.open({
                label: '查看项目类别',
...
const add = () => {
  // const url = `/opdesk/sys/master/masterDataCategoryAdd`;
...
const handleChildWindowClose = (event) => {
  if (event.data.action === 'categoryWindowClose') {
    actions.list()
  }
}
onMounted(() => {
