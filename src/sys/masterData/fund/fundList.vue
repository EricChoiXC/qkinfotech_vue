    ...
    <n-layout>
      <n-layout-content>
        ...
        <n-space justify="end" style="width: 100%">
          ...
          <commonImport
            module="masterdata/fund"
            class="com.qkinfotech.core.tendering.model.masterModels.MasterDataFund"
            @closeImportModalFunc="onCloseImportModal"
          ></commonImport>
          <commonExport module="masterdata/fund"></commonExport>
        </n-space>
    ...
    render(row) {
      return [
        h(
          ...
          {
            onClick: () => {
              // const url = `/opdesk/sys/master/masterDataFundView?fId=${encodeURIComponent(row.fId)}`;
              // window.open(url);
              layout.open({
                label: '查看资金来源',
...
const add = () => {
  // const url = `/opdesk/sys/master/masterDataFundAdd`;
...
const handleChildWindowClose = (event) => {
  if (event.data.action === 'fundWindowClose') {
    // 这里调用你的刷新列表的方法
    actions.list()
  }
}
