...
//   try {
//     const res = await axios.post( `/pm/masterdata/meetingType/load?fId=${id}`);
//     console.log("312",res.data)
  ...
  try {
    ...
    try {
      ...
      if (res.data.list.length > 0 && res.data.list[0].fId != data.value.fId) {
        ...
        // window.opener.postMessage({ action: 'meetingTypeWindowClose' }, '*'); // '*' 表示允许任何源接收消息，实际应用中应明确指定源以确保安全
